...
# --------------------------------------------------------
# Fully copied from the Multi-MAE repository
...
class MultiTaskDatasetFolder(VisionDataset):
    ...
    def __init__(
            ...
            target_transform: Optional[Dict[str, Callable]] = None,
            is_valid_file: Optional[Callable[[str], bool]] = None,
            prefixes: Optional[Dict[str, str]] = None,
            max_images: Optional[int] = None
    ) -> None:
        super(MultiTaskDatasetFolder, self).__init__(root, transform, target_transform)
        self.tasks = tasks
        ...
        samples = {
            task: make_dataset(os.path.join(self.root, f'{prefixes[task]}{task}'), class_to_idx, extensions,
                               is_valid_file)
    ...
    def apply_transform(self, sample_dict: Dict[str, Any]) -> Dict[str, Any]:
        """
        TODO: why did naming this tranform make self.transform to be None in the __get_item__
        :param sample_dict:
        :return:
        """

        assert len(sample_dict.keys()) == len(self.tasks) == len(self.__transform.keys())

        for task in self.tasks:
            sample_dict[task] = self.__transform[task](sample_dict[task])

        return sample_dict

    def target_transform(self, sample_dict: Dict[str, Any]) -> Dict[str, Any]:
        raise NotImplementedError("Target transform not implemented. im dealing with Auto Encoders i dont care :)")
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        ...
        sample_dict: Dict[str, Image] = {}
        for task in self.tasks:
            path, target = self.samples[task][index]
            sample = pil_loader(path, convert_rgb=(task == "rgb"))
            sample: Image = sample.convert("P") if "semseg" in task else sample
            sample_dict[task] = sample

        if self.__transform is not None:
            sample_dict = self.apply_transform(sample_dict)
...
class MultiTaskImageFolder(MultiTaskDatasetFolder):
    ...
    def __init__(
        ...
        super(MultiTaskImageFolder, self).__init__(root, tasks, loader,
