...
def get_transform(task: str, image_size=Optional[int]):
    if task in ["rgb", "normal", "reshading"]:
        ...
        "semantic"
    ]:  # stored as a 1 channel image (H,W) where each pixel value is a different class
    ...
    if image_size is not None:
        resize = (
            ...
            if task not in ["semantic"]
            else transforms.Resize(image_size, interpolation=InterpolationMode.NEAREST)
        )

        transform = transforms.Compose([resize, transform])
...
def default_loader(path):
    if ".npy" in path:
        ...
        raise NotImplementedError(
            "Not sure how to load files of type: {}".format(os.path.basename(path))
        )
    else:
...
def make_collate_fn(tasks: Union[List[str], str]) -> Callable:
    ...
    def my_collate_fn(
        list_of_inputs: List[Tuple[Dict[str, torch.Tensor], int]]
        ...
        }  # TODO: change this to a global var with set of tasks (?)
